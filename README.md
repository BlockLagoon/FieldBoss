...
GeoJSON is an open standard format (based on JSON format) designed for representing geographical features, along with their non-spatial attributes.  The features include points, lines, polygons, and multi-part collections.  A geo-located NFT uses the GeoJSON format to embed spatial geometries into the metadata describing the NFT.  From that metadata an NFT's location can be derived and plotted onto a map and non-spatial attributes assigned to the location.
...
* Or, download the zip file (**XRPL_FieldBoss.zip**) from the repo which is a folder containing all of the required files.
* An easy method for downloading files from a github repo is using **DownGit**   https://downgit.github.io   
Use 'https://' + 'github.com' + '/BlockLagoon/XRPL_Wave3/blob/main/XRPL_FieldBoss.zip' as the GitHub file for DownGit
<p align="right">(<a href="#top">back to top</a>)</p>
